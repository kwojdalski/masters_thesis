...
def draw_mlp(
    ...
    *,
    ...
    ax.text(
        x + w / 2, y + h + title_y_pad, title, fontsize=10, ha="center", va="bottom"
    )
...
def draw_note_box(
    ...
    if subtitle:
        ax.text(
            x + w / 2, y + h * 0.20, subtitle, fontsize=7.5, ha="center", va="center"
        )
    return {
        "left": (x, y + h / 2),
        "right": (x + w, y + h / 2),
...
def build_td3_figure():
    ...
    actor = draw_mlp(
        ax, 0.08, 0.39, 0.23, 0.19, [3, 4, 3], r"Actor network $\mu_\phi(s_t)$"
    )
    critic1 = draw_mlp(
        ax, 0.42, 0.49, 0.22, 0.14, [4, 4, 3, 1], r"Critic 1 $Q_{\theta_1}(s_t,a_t)$"
    )
    critic2 = draw_mlp(
        ax, 0.42, 0.23, 0.22, 0.14, [4, 4, 3, 1], r"Critic 2 $Q_{\theta_2}(s_t,a_t)$"
    )
    ...
    target_c2 = draw_mlp(
        ...
        0.09,
        [4, 3, 1],
        r"Target critic $Q_{\theta_2'}$",
        title_y_pad=0.008,
    )

    # TD3-specific notes / target box
    delayed = draw_note_box(
        ax,
        0.30,
        0.835,
        0.32,
        ...
        "actor updated every d critic steps",
    )
    smoothing = draw_note_box(
        ax,
        0.66,
        0.835,
        0.31,
        ...
        r"$\tilde a_{t+1}=\mu_{\phi'}(s_{t+1})+\epsilon$",
    )
    td3_target = draw_note_box(
        ax,
        0.46,
        0.05,
        0.25,
        0.085,
        "TD3 target",
        r"$y_t=r_t+\gamma(1-d_t)\min(Q'_1,Q'_2)$",
    )
    ...
    ax.plot(
        [actor["left_mid"][0], state_split_x], [s_y, s_y], color="black", linewidth=1.1
    )
    draw_poly_arrow(
        ax,
        [
            (state_split_x, s_y),
            (state_split_x, critic1["left_lower"][1]),
            critic1["left_lower"],
        ],
        lw=1.0,
    )
    draw_poly_arrow(
        ...
        [
            (state_split_x, s_y),
            (state_split_x, critic2["left_lower"][1]),
            critic2["left_lower"],
        ],
        lw=1.0,
    )
    ...
    draw_poly_arrow(
        ax,
        [actor["right_mid"], action_split],
        text=r"$\mu_\phi(s_t)$",
        text_xy=(0.33, s_y - 0.03),
        fontsize=8,
    )
    draw_poly_arrow(
        ax,
        [action_split, (0.42, critic1["left_upper"][1]), critic1["left_upper"]],
        lw=1.0,
    )
    draw_poly_arrow(
        ax,
        [action_split, (0.42, critic2["left_upper"][1]), critic2["left_upper"]],
        lw=1.0,
    )
    ax.text(
        0.475,
    ...
    draw_poly_arrow(
        ax,
        [critic1["right_mid"], q1_out],
        text=r"$Q_1(s_t,a_t)$",
        text_xy=(0.675, q1_out[1] + 0.03),
        fontsize=8,
    )
    draw_poly_arrow(
        ax,
        [critic2["right_mid"], q2_out],
        text=r"$Q_2(s_t,a_t)$",
        text_xy=(0.675, q2_out[1] - 0.03),
        fontsize=8,
    )
    ...
    ax.plot(
        [bus_x, bus_x], [bus_bot - 0.06, bus_top + 0.06], color="black", linewidth=1.2
    )
    ax.text(
        bus_x - 0.02,
        (bus_top + bus_bot) / 2,
    ...
    draw_poly_arrow(
        ax, [(bus_x, target_actor["left_mid"][1]), target_actor["left_mid"]], lw=1.0
    )
    draw_poly_arrow(
        ax, [(bus_x, target_c1["left_mid"][1]), target_c1["left_mid"]], lw=1.0
    )
    draw_poly_arrow(
        ax, [(bus_x, target_c2["left_mid"][1]), target_c2["left_mid"]], lw=1.0
    )

    # Target actor -> smoothing box -> smoothed action bus
    draw_poly_arrow(
        ax, [target_actor["top_mid"], smoothing["bottom"]], color="#444444", lw=1.0
    )
    action_bus_x = 0.952
    ax.plot(
        [action_bus_x, action_bus_x],
        [target_c2["left_upper"][1], target_actor["right_mid"][1] + 0.02],
        color="#444444",
        linewidth=1.0,
    )
    draw_poly_arrow(
        ax,
        [
            target_actor["right_mid"],
            (action_bus_x, target_actor["right_mid"][1]),
            (action_bus_x, target_c1["left_upper"][1]),
            target_c1["left_upper"],
        ],
        text=r"$\tilde a_{t+1}$",
    ...
    draw_poly_arrow(
        ax,
        [(action_bus_x, target_c2["left_upper"][1]), target_c2["left_upper"]],
        color="#444444",
        lw=1.0,
    )
    ...
    qmerge_y = 0.11
    ax.plot(
        [qprime_merge_x, qprime_merge_x],
        [qmerge_y, target_c1["right_mid"][1]],
        color="black",
        linewidth=1.0,
    )
    draw_poly_arrow(
        ax,
        [target_c1["right_mid"], (qprime_merge_x, target_c1["right_mid"][1])],
        lw=1.0,
    )
    draw_poly_arrow(
        ax,
        [target_c2["right_mid"], (qprime_merge_x, target_c2["right_mid"][1])],
        lw=1.0,
    )
    draw_poly_arrow(
        ax,
        [
            (qprime_merge_x, qmerge_y),
            (td3_target["right"][0], qmerge_y),
            td3_target["right"],
        ],
        lw=1.0,
    )
    ...
    draw_poly_arrow(
        ax,
        [
            (loss_bus_x, loss_bus_y),
            (loss_bus_x, 0.45),
            (critic1["bottom_mid"][0], 0.45),
            critic1["bottom_mid"],
        ],
        lw=1.0,
    )
    draw_poly_arrow(
        ...
        [
            (loss_bus_x, loss_bus_y),
            (critic2["bottom_mid"][0], loss_bus_y),
            critic2["bottom_mid"],
        ],
        lw=1.0,
    )
