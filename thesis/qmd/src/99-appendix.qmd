...
@sec-implementation-training-pipeline gives TD3's full training loop (@alg-td3) since TD3 is this
thesis's primary method. DDPG and PPO are baseline comparisons used in
@sec-ch-results's algorithm comparison; their training loops are recorded here for
reproducibility.
...
\begin{algorithm}
\caption{DDPG Training (follows @alg-setup)}
\begin{algorithmic}
\REQUIRE Processed environments from @alg-setup; DDPG hyperparameters
...
\begin{algorithm}
\caption{PPO Training (follows @alg-setup)}
\begin{algorithmic}
\REQUIRE Processed environments from @alg-setup; PPO hyperparameters
...
**For DDPG** (beyond the symbols already defined in @sec-implementation-training-pipeline for TD3):
...
# Bid/Ask Repricing by Instrument {#sec-appendix-bid-ask-repricing}

```{=latex}
\addcontentsline{lap}{appendices}{\protect\numberline{\thesection}Bid/Ask Repricing by Instrument}
```

@sec-execution-realism re-prices the Hypothesis 1 TD3 test action path at the
bid/ask, on AAPL, and reports that charging the spread turns the gross profit
negative. @tbl-bid-ask-repricing extends that calculation to all six
instruments, using each instrument's mean relative half-spread from
@tbl-tick-breakeven-fee and the same TD3 checkpoint's per-instrument test
rollout. The spread cost is the realised turnover of that instrument's action
path (the sum of absolute position changes) times its mean relative
half-spread; the breakeven fee is the per-step gross return divided by the
per-step turnover, exactly as in @sec-execution-realism.

```{python}
#| label: tbl-bid-ask-repricing
#| tbl-cap: "Bid/ask repricing of the Hypothesis 1 TD3 test action path, by instrument."
#| echo: false
#| warning: false
...
_repricing_path = (
    repo_root()
    / "thesis/qmd/results/pooled_td3_hft_lob_state_space_pooled_streaming_selected_dsr/peek/bid_ask_repricing.json"
)

if _repricing_path.exists():
    df = pd.DataFrame(json.loads(_repricing_path.read_text()))
    df["Symbol"] = df["symbol"]
    df["Gross PnL"] = df["gross_pnl"].apply(lambda x: f"{x:.3f}")
    df["Spread Cost"] = df["spread_cost"].apply(lambda x: f"{x:.3f}")
    df["Ratio"] = df["cost_to_profit_ratio"].apply(lambda x: f"{x:.1f}")
    df["Net PnL"] = df["net_pnl"].apply(lambda x: f"{x:.3f}")
    df["Breakeven Fee"] = df["breakeven_fee_bp"].apply(lambda x: f"{x:.4f}")
    display_df(
        df[
            [
                "Symbol",
    ...
    table_note(
        source="Author's own calculations based on the Hypothesis 1 TD3 checkpoint's "
        "per-instrument test rollouts and the half-spread in @tbl-tick-breakeven-fee.",
        note=(
            "Gross PnL and Net PnL are in net-liquidation-value units on the same "
            "scale as the test-window returns reported in Chapter VI (turnover x "
            "return, not a percentage). Spread Cost is turnover (sum of absolute "
            "position changes over the test window) times the instrument's mean "
            "relative half-spread. Ratio is Spread Cost divided by Gross PnL. Net "
            "PnL is Gross PnL minus Spread Cost, and is negative for every "
            "instrument, so the fixed action path is unprofitable under bid/ask "
            "execution on all six. Breakeven Fee, in basis points, is the "
            "per-step gross return divided by the per-step turnover. These "
            "figures re-price a fixed policy; they are not a retrained result "
            "(@sec-execution-realism)."
        ),
    )
else:
    missing_data_notice(
        "bid_ask_repricing.json not found — run: "
...
The figure below shows the six major stages of the implemented pipeline at a
glance. Not every stage's output is reported in this thesis: significance
testing and explainability are implemented and exercised, but their results are
not among the reported findings.

![The six major stages of the experimental pipeline, from data acquisition through post-training evaluation.](diagrams/fig-experimental-pipeline.svg){#fig-experimental-pipeline fig-align="center" width="3.61in" fig-pos="H"}
