...
The two quantities are often conflated and should not be: the half-spread runs
between 2.8 and 24.3 times the tick figure across these six instruments,
averaging 1.151 basis points against a tick range of 0.077 to 0.240. The tick
figure is the scale at which a minimum quote movement stops paying for itself;
the half-spread is the cost actually incurred when crossing to trade. Since the
fee sweep in @sec-h2-transaction-cost-sensitivity locates the behavioural break
between 0.01 and 0.1 basis points, the learned margin is below the tick scale
and roughly an order of magnitude below the half-spread.
