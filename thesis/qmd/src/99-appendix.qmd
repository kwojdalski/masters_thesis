...
$QWM_t = \tfrac{V^{ask}_0P^{bid}_0+V^{bid}_0P^{ask}_0}{V^{bid}_0+V^{ask}_0}$: queue-weighted midpoint (legacy feature name `microprice`);
...
\texttt{hft\_microprice} & Fair value & $QWM_t$ & --- & $\checkmark$ & Stoikov (2018) \\
\texttt{hft\_microprice\_divergence} & Fair value & $QWM_t-M_t$ & --- & $\checkmark$ & --- \\
...
{\footnotesize\itshape\parindent0pt\raggedright\relax\textbf{Source:} author's own compilation. Formulas are implementations or adaptations; citations identify conceptual sources where applicable. A dash denotes an author's construction or no direct originating formula.\par}
\end{landscape}
```
