...
```{=latex}
\addcontentsline{lap}{appendices}{Tick-Size Break-Even Fee by Instrument}
```
...
#| tbl-cap: "Break-even transaction fee -- the proportional cost at which a one-sided minimum-tick move stops covering the fee -- by mean instrument price over the evaluation window."
#| echo: false
#| warning: false
...
if _tick_path.exists():
    ...
    df["Break-Even Fee"] = df["breakeven_fee_bp"].apply(lambda x: f"{x:.3f} bp")
    display_df(df[["Symbol", "Mean Price", "Break-Even Fee"]])
    table_note(
        source="Author's own calculations based on DataBento Nasdaq MBP-10 data.",
        note=(
            "Break-even fee = (half of the one-cent minimum price increment / "
            "mean mid-price) x 10,000, in basis points. Mean price is computed "
            "over the 2 March 2026 evaluation session."
        ),
    )
else:
    missing_data_notice(
