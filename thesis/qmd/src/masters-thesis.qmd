---
title: Microstructure-Aware Reinforcement Learning for High-Frequency Trading
author: Krzysztof Wojdalski
date: today
format:
  pdf:
    documentclass: pracamgrwne
    latex-engine: xelatex
    toc: true
    toc-depth: 3
    number-sections: true
    output-dir: ../../build
    linestretch: 1.5
    header-includes:
      - \usepackage[utf8]{inputenc}
      - \usepackage{amsthm}
      - \usepackage{rotating}
      - \usepackage{algorithm}
      - \usepackage{algpseudocode}
      - \usepackage{amsmath}
      - \usepackage{booktabs}
      - \usepackage{longtable}
      - \nralbumu{310284}
      - \speciality{Economics}
      - \opiekun{dr Pawel Sakowski}
      - \keywords{Reinforcement Learning, High-Frequency Trading, Order Book, Single-Asset Trading, TD3}
      - \dziedzina{14.3 Economics}
      - \klasyfikacja{G11, G17, C63}
      - \tytulang{Microstructure-Aware Reinforcement Learning for High-Frequency Trading}
  docx:
    embed-resources: true
  html:
    toc: true
    toc-depth: 2
    number-sections: true
    number-depth: 2
    toc-location: right
bibliography: ../../bibliography/library.bib
post-render:
  - post-render-script.py
editor_options:
  chunk_output_type: console
  latex_engine: xelatex
jupyter:
  kernelspec:
    display_name: Python 3
    language: python
    name: python3
thesis_runtime_config:
  scenario: "src/configs/scenarios/aapl_td3_hft_lob_state_space.yaml"
  feature_config_primary: "src/configs/features/hft_lob_features_state_space.yaml"
  feature_config_baseline: "src/configs/features/hft_lob_features.yaml"
  experiment_name: "TRADINGENV_TD3_AAPL_HFT_STATE_SPACE"
---

\theoremstyle{definition}
\newtheorem{definition}{Definition}
\newpage

\begin{abstract}

This thesis studies whether deep reinforcement learning can produce robust,
risk-adjusted trading performance in a realistic high-frequency setting. The
work focuses on continuous portfolio exposure control from order-book-derived
features and addresses practical constraints that often invalidate paper-trading
results.

The empirical framework implements a TD3 agent on high-frequency AAPL data and
evaluates it against benchmark strategies under explicit assumptions about
transaction costs and execution. Performance is assessed with both financial
metrics and statistical validation to separate genuine signal from noise and
overfitting.

The main contribution is an end-to-end, reproducible pipeline that links market
feature engineering, reward design, agent training, and robustness testing in a
single evaluation framework for algorithmic trading research.

\end{abstract}

\pagebreak


{{< include ./01-00-00-introduction.qmd >}}

<!-- {{< include ./01-01-00-statistical-arbitrage.qmd >}} -->
<!-- {{< include ./01-01-01-statistical-arbitrage-vs-other-types.qmd >}} -->
<!-- {{< include ./01-01-02-critical-parameter-selection.qmd >}} -->
{{< include ./01-02-00-scope-and-objectives.qmd >}}
<!-- {{< include ./01-03-00-methodological-framework.qmd >}} -->
{{< include ./01-04-00-thesis-organization-and-chapter-overview.qmd >}}

\newpage
{{< include ./02-00-00-literature-review.qmd >}}
{{< include ./02-02-02-empirical-evidence.qmd >}}
{{< include ./02-02-04-microstructure-theory.qmd >}}
{{< include ./02-03-00-decision-paradigms.qmd >}}
{{< include ./02-03-03-02-seminal-works.qmd >}}
{{< include ./02-03-03-01-key-concepts.qmd >}}
{{< include ./02-03-03-03-rl-applications.qmd >}}

<!-- commented out — financial theory sections not used in current thesis scope -->
<!-- {{< include ./02-01-01-01-classical-models.qmd >}} -->
<!-- {{< include ./02-01-01-02-modern-portfolio-theory-and-capm.qmd >}} -->
<!-- {{< include ./02-01-01-03-factor-models.qmd >}} -->
<!-- {{< include ./02-01-02-00-modern-approaches.qmd >}} -->
<!-- {{< include ./02-01-03-00-critical-analysis.qmd >}} -->
<!-- {{< include ./02-02-00-relevant-financial-indicators.qmd >}} -->
<!-- {{< include ./02-02-03-market-anomalies.qmd >}} -->

<!-- Chapter 3 content merged into Chapter 2 (decision paradigms) and Chapter 5 (implementation architecture) -->
<!-- {{< include ./03-01-00-machine-learning.qmd >}} -->
<!-- {{< include ./03-02-01-01-components-of-ats.qmd >}} -->
<!-- {{< include ./03-02-02-00-rule-based-trading.qmd >}} -->
<!-- {{< include ./03-02-03-01-trading-based-on-forecasts.qmd >}} -->
<!-- {{< include ./03-02-03-02-training-on-labelled-data.qmd >}} -->
<!-- {{< include ./03-02-03-03-direct-optimization.qmd >}} -->

\newpage
{{< include ./04-00-reinforcement-learning.qmd >}}
{{< include ./04-03-rl-categories.qmd >}}
{{< include ./04-02-timeline-of-new-rl-algorithms.qmd >}}

\newpage
{{< include ./05-00-design-of-the-trading-agent.qmd >}}
{{< include ./05-01-action-space.qmd >}}
{{< include ./05-02-state-space.qmd >}}
{{< include ./05-03-reward-function.qmd >}}
{{< include ./05-04-value-function.qmd >}}
{{< include ./05-05-policy.qmd >}}
{{< include ./05-06-discount-factor.qmd >}}
{{< include ./05-07-step-size.qmd >}}

\newpage
{{< include ./06-00-implementation-of-the-trading-agent.qmd >}}
{{< include ./06-01-data-preparation.qmd >}}
{{< include ./06-02-code.qmd >}}
<!-- merged into 06-02 -->
<!-- {{< include ./06-03-td3-training-pipeline.qmd >}} -->

\newpage
{{< include ./07-00-empirical-evaluation-and-performance-analysis.qmd >}}
{{< include ./07-01-statistical-validation.qmd >}}
{{< include ./07-02-robustness-assessment.qmd >}}
{{< include ./07-03-performance-evaluation.qmd >}}

\newpage
{{< include ./08-00-conclusions-and-future-work.qmd >}}
{{< include ./08-01-summary-of-findings.qmd >}}
{{< include ./08-02-limitations-and-future-research.qmd >}}
{{< include ./08-03-implications-for-trading-systems.qmd >}}
{{< include ./08-04-recommendations-for-practitioners.qmd >}}
{{< include ./08-05-conclusion.qmd >}}

{{< include ./98-bibliography.qmd >}}
{{< include ./99-appendix.qmd >}}
