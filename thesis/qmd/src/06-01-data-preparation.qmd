...
configuration-driven experimentation, where many scenarios may be tested in
sequence.

### Feature Engineering Pipeline

Feature engineering is governed by an external, configuration-driven feature
pipeline. Rather than hard-coding feature transformations inside the training
logic, the workflow defines feature sets declaratively. This design supports
rapid experimentation and improves reproducibility, because the exact state
representation can be versioned and compared across scenarios.
...
The configuration-driven approach also enables feature ablation studies. For
example, the research can compare a comprehensive feature set with a reduced,
non-redundant subset while keeping the rest of the experimental setup unchanged.
