...
* Model-Based approaches - These methodologies operate under the assumption
that an environmental model is known a priori. Action selection proceeds
through systematic search and planning within this model framework. The
Markov Decision Process (MDP) exemplifies this approach, as it necessitates
knowledge of both the Markov transition matrix and the reward function.
...
arbirary values and then updates $Q(s, a)$ and $V(s)$ values until they
...
<!-- \end{algorithm} -->
<!-- ![Policy Iteration Algorithm. Based on @Sutton2017](../../figures/policy_iteration.png) -->
