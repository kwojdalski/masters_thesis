...
def calculate_twap_dsr(
    ...
    """Calculate DSR for TWAP strategy (linear position buildup from 0 to 1)."""
    if price_column not in df_prices.columns:
        if "close" in df_prices.columns:
            logger.warning(
            ...
            raise ValueError(
                f"TWAP DSR requires '{price_column}' or 'close' in df_prices columns."
            )
