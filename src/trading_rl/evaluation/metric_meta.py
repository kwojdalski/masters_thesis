...
METRIC_META: list[MetricMeta] = [
    ...
    MetricMeta(
        ...
        ".4%",
        "Risk",
        "Per-bar standard deviation computed only on negative returns: √(mean(min(r,0)²)). "
        "Not annualised — this is the actual value used in the Sortino ratio denominator.",
        is_pct=True,
    ),
    MetricMeta(
        ...
        ".2%",
        "Risk",
        "downside_deviation scaled to annual frequency: downside_deviation × √periods_per_year. "
        "Informational only — not used in sortino_ratio.",
        is_pct=True,
    ),
    MetricMeta(
        "tail_ratio",
