...
def generate_upward_drift_pattern(
    ...
    *,
    ...
    # Enforce shallow pullbacks relative to the close series' own running
    # peak. Comparing against the un-lagged drift_curve instead would clamp
    # away the rolling-mean's smoothing lag on every step regardless of
    # noise, deleting all volatility (see #483).
    floor = pullback_floor * np.maximum.accumulate(close_prices)
