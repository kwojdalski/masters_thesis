...
ReturnOnInvestment <- function(l_quotes, i_period)
{
  ...
  returnOnInvestment <- c((1 : i_period)*NA, c_differences / l_quotes[1:length(c_differences)]) + 1
...
AttributesAll <- function(df_currencies, c_currencies, i_delay, base_series = list('price','return')){
...
{                        
  ...
  for(ch_asset in c_assets)
  {
    ...
    df_attributes[paste("EMA3_zwrot_", ch_asset, sep = "")] <- EMA(i_return, n = 3, wilder = TRUE)
    ...
    df_attributes[paste("EMA10_zwrot_", ch_asset, sep = "")] <- EMA(i_return, n = 10, wilder = TRUE)
    ...
    #TEMA15 dniowa
    Tema3 <- 3*EMA(kurs, n=3) - (3*(EMA(EMA(kurs, n=3), n=3))) + EMA(EMA(EMA(kurs, n=3), n=3), n=3)
    df_attributes[paste("TEMA3_", ch_asset, sep = "")] <- Tema3[1:nrow(df_attributes)]
    #TEMA60dniowa
