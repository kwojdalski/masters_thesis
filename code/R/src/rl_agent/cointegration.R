...
GetCointPairs <- function(df_currencies, c_currencies, learn_period, cval) {
  ...
  for (i in 1:NCOL(df_combn)) {
    if (
      ca.jo(df_currencies[df_combn[, i]])@teststat[2] <
        ca.jo(df_currencies[df_combn[, i]])@cval[2, cval]
    ) {
      ...
      #colnames(df_combn) # saves columns of uncointegrated pairs
      rejected_combn <- c(rejected_combn, i)
    }
  }
...
CointSpreads <- function(
  ...
  for (i in (1:(ncol(coint_pairs)))) {
    ...
    train_spread <- coint_vec[1] *
      train_sample[, 1] +
      coint_vec[2] * train_sample[, 2]
    test_spread <- coint_vec[1] *
      test_sample[, 1] +
      coint_vec[2] * test_sample[, 2]
