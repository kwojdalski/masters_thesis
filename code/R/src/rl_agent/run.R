...
train_features <- as.data.frame(l_attributes[pair_ind])[
  c(train_period[1]:train_period[2]),
]

test_pair <- df_currencies[c(test_period[1]:test_period[2]), coint_pair]
test_spread <- test_spreads[[pair_ind]]
test_features <- as.data.frame(l_attributes[pair_ind])[
  c(test_period[1]:test_period[2]),
]
...
train_spread %>%
  ...
  {
    ggplot(., aes(seq(1:nrow(.)), y = .)) + geom_line()
  }
test_spread %>%
  {
    data.frame(x = .)
...
pretrain_return <- Ret(
  train_pair,
  coef,
  tail(pretrained_pos, nrow(train_pair)),
  cost = cost
)
PlotAgentResult(
  train_pair,
