...
ret_asset <- function(asset, previous_ret = NULL) {
  # returns the return of the buy&hold strategy for a single asset
...
Ret <- function(pair, coef, position, cost, previous_ret = NULL, ...) {
  ...
  for (i in 2:n) {
    ...
    if (position[i] == -1 & position[i - 1] != 0 & diff_pos[i - 1] != 0) {
      additive[i] <- ((-(coef[1] *
        (pair[i, 1] - pair[i - 1, 1]) +
        coef[2] * (pair[i, 2] - pair[i - 1, 2])) /
        (pair[i - 1, 1] + abs(coef[2]) * pair[i - 1, 2])) +
        1) *
        additive[i - 1]
    ...
    } else if (position[i] == 1 & position[i - 1] != 0 & diff_pos[i - 1] != 0) {
      #position taken (long)
      additive[i] <- ((+(coef[1] *
        (pair[i, 1] - pair[i - 1, 1]) +
        coef[2] * (pair[i, 2] - pair[i - 1, 2])) /
...
PlotAgentResult <- function(pair, spread, additive, strategy_name) {
  ...
  df_all <- data.frame(
    ...
    asset2 = asset2,
    spread = spread
  ) #center because cant plot geom_bar() not from the center line..
...
diff_sharpe_ratio <- function(returns, time = NULL, eta) {
  ...
  assert_that(
    length(returns) > 0,
    msg = "returns array can\'t be shorter than 1"
  )
