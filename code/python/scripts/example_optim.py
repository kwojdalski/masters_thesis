...
# version of the actor to be used within the loss module and during evaluation.
...
# Here comes the data part: we need a data collector to easily get batches of data
# and a replay buffer to store that data for training.
...
for name, param in policy_explore.named_parameters():
    print(f"{param.data.mean().item():.4f}")
    ...
    if len(rb) > init_rand_steps:
        ...
        for _ in range(optim_steps):
            ...
            for name, param in policy.named_parameters():
                torchrl_logger.info(f"{name}: {param.data.mean().item():.4f}")
            torchrl_logger.info("=====")
            if i % 10:
