...
``logs/td3_hft_lob_state_space_pooled_streaming_selected_dsr/per_symbol/<SYM>/
test_<SYM>_rollout.parquet``. That directory is a local, gitignored run
directory (see ``logs/*`` in .gitignore), not a thesis-tracked artifact, so
this script reads from it and exports the small derived summary to
...
Requires the per-symbol rollout parquets under
``logs/td3_hft_lob_state_space_pooled_streaming_selected_dsr/per_symbol/``,
which are produced by evaluating the Hypothesis 1 TD3 checkpoint per
instrument (already run; not reproduced by this script).
"""
...
_ROLLOUT_DIR = (
    _REPO_ROOT / "logs/td3_hft_lob_state_space_pooled_streaming_selected_dsr/per_symbol"
)
...
def main() -> None:
    ...
    for symbol in _SYMBOLS:
        rollout_path = _ROLLOUT_DIR / symbol / f"test_{symbol}_rollout.parquet"
