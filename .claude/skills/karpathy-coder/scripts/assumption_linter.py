...
ASSUMPTION_SIGNALS = [
    (
        ...
        "assumption-just",
        "'just' often hides complexity. What's being skipped?",
    ),
    (
...
MISSING_CLARIFICATION = [
    (
        re.compile(
            r"\b(?:export|import|save|load|fetch|send)\b.*\b(?:data|file|users)\b", re.I
        ),
        "missing-format",
        "Export/save/fetch mentioned but format not specified (JSON? CSV? API?)",
    ),
    (
        re.compile(r"\b(?:fix|improve|optimize|refactor|update)\b", re.I),
        "vague-action",
        "Vague action verb. What specifically changes? What's the measurable improvement?",
    ),
    (
        re.compile(
            r"\b(?:handle|deal with|take care of)\b.*\b(?:error|edge|case)\b", re.I
        ),
        "vague-error-handling",
        "Error handling mentioned vaguely. Which errors? What behavior?",
    ),
    (
        re.compile(
            r"\b(?:the user|users)\b(?!.*\b(?:who|which|specific|certain|admin|role)\b)",
            re.I,
        ),
        "unscoped-user",
        "Which user(s)? All? Specific role? Authenticated only?",
    ),
]
...
NO_VERIFICATION = [
    (
        re.compile(
            r"^(?:(?!(?:test|verify|check|assert|confirm|ensure|validate)).)*$", re.I
        ),
        "no-verification",
        "No verification step found in this block. How will you know it works?",
    ),
]
...
def lint_text(text, source_name="stdin"):
    ...
    for block in plan_blocks:
        has_verify = bool(
            re.search(
                r"\b(?:test|verify|check|assert|confirm|ensure|validate)\b", block, re.I
            )
        )
        if not has_verify:
            findings.append(
...
def main():
    ...
    p.add_argument(
        "input", nargs="?", default="-", help="Markdown file to lint, or - for stdin"
    )
    ...
    result = {
        ...
        "verdict": "CLEAN"
        if len(findings) == 0
        else ("REVIEW" if len(findings) < 5 else "CLARIFY"),
        "findings": findings,
    }
