...
- Everything that populates `thesis/qmd/results/` runs through one entry point: `uv run thesis-export` (`src/masters_thesis/export/`). `uv run thesis-export --list` shows every stage and which are runnable here -- a stage whose inputs are absent (no `mlruns/`, no `data/prepared/`) is skipped, not failed. A chapter still showing placeholders despite a finished training run usually means the relevant stage was skipped or never re-run.
